define(["dojo","dojox/json/ref"], function(dojo,ref) {
dojo.provide("dojox.data.JsonStore");
...
dojo.declare("dojox.data.JsonStore", null, {
	labelAttribute: "name",
	idAttribute: "id",
	...
	setData: function(data){
		...
		this._index = {};
		this._refOptions = {index: this._index, idAttribute: this.idAttribute, idPrefix:''};
...
//		console.log("deleting: ", item);	
		var updated = [];
		/*
		dojoc.dmachi.data.JsonStore.deleteReferences(item, this._data, this._refMap, {idAttribute: this.idAttribute, parentSetCallback: dojo.hitch(this, function(parent, prop, i){
			//console.log("Removed Item reference from: ", parent, prop, i);
			updated.push(arguments);
		})});
		*/

		var qrs = '$..[?' + this.idAttr + '=="' + id +'"]' ;
		console.log("Delete query: " + qrs);
	//	var res = dojox.json.query(qrs, this._data);
	//	dojo.forEach(res,function(ref){
	//		console.log("Ref: ", ref);
	//	},this);
...
dojox.data.JsonStore.deleteReferences = function(item, data, refmap,  options){
	...
	if (refmap && id && refmap[id]){
		console.log("Found refmap && refmap[id]");
		...
		if (dojo.isArray(data)){
				...
				if (dojo.isObject(data[i])){
					if (data[i][idAttr]){
						...
						if (data[i][idAttr]==id){
							...
							console.log("    items don't match");
							if (refmap){
								console.log("found refmap: ", refmap);
							...
							console.log("recuse to delete item");
						 	dojoc.dmachi.data.JsonStore.deleteReferences(item, data[i], refmap, options); 
						}
					}else{
						console.log("something else: ", data[i]);
						 dojoc.dmachi.data.JsonStore.deleteReferences(item, data[i], refmap, options); 
					}
				}else{
					console.log("something else even further");
					if (dojo.isArray(data[i])){ dojoc.dmachi.data.JsonStore.deleteReferences(item, data[i], refmap, options);  }
				}
			}
			...
			for (var prop in data){
				...
				if (prop!="__proto__" && data[prop]!=null && data[prop]!=undefined &&dojo.isObject(data[prop]) && data[prop][idAttr]){
					if(data[prop][idAttr]==id) {
						...
						dojoc.dmachi.data.JsonStore.deleteReferences(item, data[prop], refmap, options);		
					}
				}else{
