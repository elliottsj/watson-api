//>>built
define("gridx/modules/move/Cell", [
	"dojo/_base/declare",
	"dojo/_base/array",
	"dojo/_base/lang",
	"../../core/_Module"
	...
	declare('gridx.modules.move.Cell', _Module, {
		...
		_checkAccept: function(cellRange, targetRowIndex, targetColIndex){
			var checker = this.arg('checkCellMoveAccept'),
			...
			for(i = start.row; i <= end.row; ++i){
				...
				for(j = start.column; j <= end.column; ++j){
					...
					if(!col ||				//No such column in grid
						...
						(idAttrs && array.indexOf(idAttrs, col.field()) >= 0) ||		//Field is ID
		...
		_doMove: function(cellRange, targetRowIndex, targetColIndex){
			...
			model.when([{
				...
				if(!copy){
					...
					var emptyValue = this.arg('getEmptyValue');
		...
		_getDiffRange: function(cellRange, targetRowIndex, targetColIndex){
			...
			if((targetRowIndex >= start.row * 2 - end.row + 1 && targetRowIndex <= end.row) ||
				(targetColIndex >= start.column * 2 - end.column + 1 && targetColIndex <= end.column)){
